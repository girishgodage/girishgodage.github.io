...
If one of the functionality fails to work, it could potentially take down the entire app
If a third party service breaks, it could potentially break your entire app
You will be forced to do everything in one language, one version and pretty much will have to use the same libraries. You won't be able to leverage a diverse ecosystem
Your database will break, users will find that the flash sale discount that he just applied to a TV isn't valid anymore
Search performance is poor as we have to pull information from a single database that will grow extremely large as the ecommerce platform usage increases
